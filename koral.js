		...
		{ 
			if ($(this).find("code").length > 0)
			{
				$(this).toggleClass("prettyprint", true);
				 ...
				 setTimeout(function(){ prettyPrint(); }, 2);
			}
		});
...
var KoralArticle = function(article) {
	...
	this.update = function(paragraph, newcode)
	{
		...
		else
		{
			...
			{
				var dom = d.get(i);
				if ($(dom).clone().wrap('<div/>').parent().html().trim().length === 0) continue;
...
var KoralInternal = {
	...
	importScripts: function(onLoadCallback) {
		...
		if (url == null) throw "Invalid koral script referencing";
		...
		var loadCallback = function() {
			loadCounter--;
			if (loadCounter <= 0) onLoadCallback();
		};
		...
		loadScripts(scriptsFirstPass, function() { loadScripts(scriptsSecondPass, loadCallback); });
	},
				...
				$(this).find("g[data-url]").each(function (i, v) 
				{
					...
					if (drawConf.type === "points")
					{
						...
						for (var i=0; i<data.length; i++)
						{
							...
							if (normx < 0 || normx > 1 || normy < 0 || normy > 1 || isNaN(normx) || isNaN(normx)) continue;
